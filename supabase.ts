...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error(
    "Supabase configuration is missing. Please check your app.json file."
  );
}
...
export const database = {
  ...
  addLearningActivity: async (activity: {
    ...
    const { data, error } = await supabase.from("learning_activities").insert([
      {
        ...activity,
  ...
  getStreakInfo: async (userId: string) => {
    try {
      // Call the calculate_user_streaks function
      const { data, error } = await supabase.rpc("calculate_user_streaks", {
        p_user_id: userId,
        p_start_date: new Date(new Date().setMonth(new Date().getMonth() - 2))
          .toISOString()
          .split("T")[0],
      ...
      // Transform the activity dates into marked dates format
      const markedDates = (data?.activity_dates || []).reduce(
        (acc: any, date: string) => {
          acc[date] = {
            marked: true,
            dotColor: "#8A97FF",
      ...
      // Add today's date if it's not already marked
      const today = new Date().toISOString().split("T")[0];
      if (!markedDates[today]) {
        markedDates[today] = {
          selected: true,
          selectedColor: "#8A97FF",
          customTextStyle: {
            color: "#FFFFFF",
            fontWeight: "bold",
          },
