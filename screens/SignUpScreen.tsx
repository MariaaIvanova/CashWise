...
export default function SignUpScreen({ navigation }: SignUpScreenProps) {
  ...
  const validatePassword = (pass: string) => {
    ...
    if (!requirements.number)
      missingRequirements.push("Паролата трябва да съдържа поне една цифра");
    if (!requirements.minLength)
      missingRequirements.push("Паролата трябва да е поне 8 символа");
    ...
    try {
      ...
      if (err instanceof Error) {
        if (err.message.includes("User already registered")) {
          ...
          errorMessage =
            "Паролата трябва да съдържа малки и главни букви, и цифри";
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (data?.url && Platform.OS !== "web") {
        ...
        console.log(
          "Web platform - browser will handle redirect automatically"
        );
      }
    } catch (err) {
      ...
      {!areAllRequirementsMet() && password.length > 0 && (
        <View style={styles.requirementsContainer}>
          <View style={styles.requirementRow}>
            <FontAwesome
              name={
                passwordRequirements.minLength ? "check-circle" : "circle-o"
              }
              size={16}
          ...
          <View style={styles.requirementRow}>
            <FontAwesome
              name={
                passwordRequirements.lowercase ? "check-circle" : "circle-o"
              }
              size={16}
          ...
          <View style={styles.requirementRow}>
            <FontAwesome
              name={
                passwordRequirements.uppercase ? "check-circle" : "circle-o"
              }
              size={16}
