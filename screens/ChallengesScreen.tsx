...
const ChallengesScreen = () => {
  ...
  const updateUserXP = async (
    ...
    try {
      ...
      if (challengeId) {
        try {
          ...
          if (error.message === "Challenge already completed today") {
            Alert.alert(
              "Вече изпълнено",
              "Вече сте получили XP за това предизвикателство днес."
            );
            return false;
          }
          throw error;
      ...
      Alert.alert(
        "Грешка",
        "Неуспешно обновяване на XP. Моля, опитайте отново."
      );
      return false;
    }
  };
  ...
  const handleInviteFriend = async () => {
    try {
      ...
      if (now - lastInviteTime < INVITE_COOLDOWN) {
        Alert.alert(
          "Моля, изчакайте",
          "Трябва да изчакате малко преди да изпратите нова покана."
        );
        return;
      }
      ...
      if (userError || !user) {
        Alert.alert(
          "Грешка",
          "Моля, влезте в профила си, за да поканите приятел"
        );
        return;
      }
      ...
      const { data: userProfile, error: userProfileError } = await supabase
        .from("profiles")
        .select("name, monthly_invitation_count")
        .eq("id", user.id)
        .maybeSingle();
      ...
      if (userProfile?.monthly_invitation_count >= 5) {
        Alert.alert(
          "Лимит достигнат",
          "Достигнали сте месечния лимит от 5 покани. Моля, опитайте отново следващия месец."
        );
        return;
      }

      // Create invitation record
      const invitationCode = await (database as any).generateInvitationCode();
      ...
      if (invitationError) {
        ...
        Alert.alert(
          "Грешка",
          "Възникна проблем при създаване на поканата. Моля, опитайте отново."
        );
        return;
      }

      // Update monthly invitation count
      const { error: updateError } = await supabase
        .from("profiles")
        .update({
          monthly_invitation_count:
            (userProfile?.monthly_invitation_count || 0) + 1,
          last_invitation_reset: new Date().toISOString(),
        })
        .eq("id", user.id);

      if (updateError) {
        console.error("Error updating invitation count:", updateError);
        // Continue anyway as the invitation was created
      }
      ...
      if (
        ...
        Alert.alert(
          "Грешка",
          "Имейл услугата не е конфигурирана правилно. Моля, свържете се с поддръжката."
        );
        return;
      }
      ...
      const templateParams = {
        ...
        message:
          "Присъедини се към CashWise и нека заедно да научим повече за финансите!",
        invitation_code: invitationCode,
      };
      ...
      Alert.alert(
        "Грешка",
        "Възникна проблем при изпращане на поканата. Моля, опитайте отново."
      );
    } finally {
      setIsInviting(false);
    }
      ...
      if (attempts) {
        ...
        const resultMap: Record<string, TestResult> = {
          impulsive: {
            ...
            description:
              "Парите за теб са начин да се наслаждаваш на живота, но внимавай – прекаленият импулс може да води до стрес и дългове. Помисли за по-дългосрочни цели и създаване на бюджет.",
            tips: [
          ...
          balanced: {
            ...
            description:
              "Имаш разумно отношение към парите, но може биsometimes се колебаеш. Усъвършенствай планирането и създай ясни финансови цели.",
            tips: [
          ...
          strategic: {
            ...
            description:
              "Ти си дисциплиниран и гледаш на парите като на инструмент, а не само източник на удоволствие. Това е стабилна основа за финансово благополучие.",
            tips: [
      ...
      const challengesList: Challenge[] = [
        {
          ...
          description:
            "Изпрати покана на приятел да се присъедини към CashWise (еднократно на месец)",
          xp: 100,
          icon: "person-add",
        ...
        },
        {
          ...
          title: "Дневна серия",
          description:
            "Завърши поне един урок и тест днес, за да поддържаш серията си",
          xp: 25,
          icon: "flame",
          ...
          action: async () => {
            ...
            try {
              ...
              if (!user) {
                Alert.alert(
                  "Грешка",
                  "Моля, влезте в профила си, за да печелите XP"
                );
                return;
              }
              ...
              const hasLesson = activities?.some(
                (a) => a.activity_type === "lesson"
              );
              const hasQuiz = activities?.some(
                (a) => a.activity_type === "quiz"
              );
              ...
              Alert.alert(
                "Грешка",
                "Възникна проблем при проверка на дневната серия. Моля, опитайте отново."
              );
            }
          },
        },
  ...
  const handleTestComplete = async (result: TestResult) => {
    try {
      ...
      if (!user) {
        Alert.alert(
          "Грешка",
          "Моля, влезте в профила си, за да запазите резултата"
        );
        return;
      }
      ...
      const { data: profile, error: profileError } = await database.getProfile(
        user.id
      );
      if (profileError) throw profileError;
              ...
              <View style={styles.resultHeaderContent}>
                ...
                <View style={styles.resultHeaderInfo}>
                  <View style={styles.resultIconContainer}>
                    <Ionicons
                      name={
                        testResult.type === "impulsive"
                          ? "flash-outline"
                          : testResult.type === "balanced"
                          ? "scale-outline"
                          : "trending-up-outline"
                      }
                      size={24}
                  ...
                  <Text style={[styles.resultTitle, { color: "#fff" }]}>
                    {testResult.title}
                  </Text>
                </View>
              </View>
              ...
              <View style={styles.resultContent}>
                ...
                <View style={styles.tipsSection}>
                  <View style={styles.tipsHeader}>
                    ...
                    />
                    <Text
                      style={[styles.tipsTitle, { color: testResult.color }]}
                    >
                  ...
                  {testResult.tips.map((tip, index) => (
                    <View key={index} style={styles.tipCard}>
                      <View
                        style={[
                          styles.tipNumber,
                          { backgroundColor: `${testResult.color}15` },
                        ]}
                      >
                        <Text
                          style={[
                ...
                <View style={styles.resultNote}>
                  ...
                  <Text style={styles.resultNoteText}>
                    Този тест може да се направи само веднъж, за да се запази
  ...
  return (
    <View style={[styles.container, { backgroundColor: "#FFFFFF" }]}>
      ...
      >
        ...
        <Text style={[styles.headerTitle, { color: "#333333" }]}>
